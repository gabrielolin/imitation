...
_timing_hooks: Dict[str, Callable[[float], None]] = {}
    ...
    def train(
        ...
        *,
        ...
        def process_batch():
            ...
            self.optimizer.step()
            if "optimizer_step" in _timing_hooks:
                _timing_hooks["optimizer_step"](time.perf_counter() - _opt_t0)
        ...
        for (
            ...
            if "loss_calc" in _timing_hooks:
                _timing_hooks["loss_calc"](time.perf_counter() - _lc_t0)
            ...
            if loss.requires_grad:
                _bw_t0 = time.perf_counter()
                loss.backward()
                if "backward" in _timing_hooks:
                    _timing_hooks["backward"](time.perf_counter() - _bw_t0)
            else:
                # Warn if loss does not require grad (e.g., final incomplete batch)
                import warnings
                warnings.warn("[BC] Skipping backward on batch: loss does not require grad (likely final incomplete batch)")
